...
if __name__ == "__main__":
    ...
    parser.add_argument("--show-agent-graph", action="store_true", help="Save agent graph as PNG")
    ...
    parser.add_argument("--realizedgains", type=str, help="Comma-separated list of realized gains of holdings")
    ...
    if args.portfolio and args.costbasis and args.realizedgains:
        ...
        realized_gains = [float(realizedgains.strip()) for realizedgains in args.realizedgains.split(",")]
        if len(tickers) == len(positions) == len(cost_basis) == len(realized_gains):
            _positions = {}
            _realized_gains = {}
            for ticker, position, cb, rg in zip(tickers, positions, cost_basis, realized_gains):
                _positions[ticker] = {
                    "long": float(position),
                    "short": 0.0,
                    "long_cost_basis": cb,
